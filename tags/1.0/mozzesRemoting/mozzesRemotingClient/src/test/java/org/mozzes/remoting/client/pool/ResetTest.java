...
public class ResetTest extends TestCase {
	...
	/**
	 * Sends two same objects, but with their state modified between two successive calls. This will show us whether
	 * java is sending only references through network, or is it always sending whole fat objects.
	 */
	@Test
	public void testSendModifiedAction() throws RemotingException {
		...
		try {
			...
			Map<? extends Object, ? extends Object> receivedParams1 = ((RemotingAction) mockupServer
					.getLastSentObject()).getParams();
			...
			assertNull(mockupServer.getLastError());
			assertTrue(mockupServer.getLastSentObject() instanceof RemotingAction);
			Map<? extends Object, ? extends Object> receivedParams2 = ((RemotingAction) mockupServer
					.getLastSentObject()).getParams();
