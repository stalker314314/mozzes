...
/**
 * Handle incoming messages from the server. Also, it keeps a list of client's requests in order to successfully
 * return response to requested client action. In case of lost communication channel with server, all client's requests
 * are notified about this event so they can stop waiting to an answer/response from the server.
 * 
...
public class MozzesClientHandler extends SimpleChannelUpstreamHandler {
	 ...
	 * @param synchronizedRequestsAndResponses holds client requests identified by its ID.  
	 */
	 ...
	 * who haven't yet got their response and when communication with server is lost they have to be notified so they don't
	 * wait anymore.
	 */
