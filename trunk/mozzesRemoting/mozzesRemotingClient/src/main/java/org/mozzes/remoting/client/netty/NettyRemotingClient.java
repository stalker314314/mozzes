...
/**
 * Remoting client which sits on Netty NIO client. Using Netty mechanism client dispatch remote actions to the server for
 * execution. Simple spoken, a client is responsible for connecting, disconnecting and executing remote actions.
 * 
 * 
...
public class NettyRemotingClient extends RemotingClientListener implements RemotingClient {
	...
	/**
	 * Service to dispatch client remote actions to the server. It has <strong>>one</strong>
	 * worker so it ensures it can not happen simultaneous write on a single communication channel.
	 */
	private ExecutorService service = Executors.newFixedThreadPool(1);

	/**
	 * It allows that one client can send more remote actions on execution, so it doesn't have to wait
	 * on a single action response. This is a map which keeps track which action correspond to which respond. 
	 */
	 ...
	 * 
	 * @param configuration - remoting configuration used to create stuff for the client. Also, it has parameters for
	 * the connection.
	 */
