...
public class StressTest extends TestCase {
  ...
  /**
   * Pool stress test with only 2 clients in pool and 100 threads executing 100 times each. Every action takes 1 ms, so
   * this method should end in minimally (97 * 97) / 2 active clients ~= 5s (6-15s in practice)
   */
  @Test
   ...
   * 
   * @param poolMaxSize
   *          Max size of the pool to be configured with
   * @param numThreads
   *          Number of threads to spawn to execute client actions
   * @param loopsInThread
   *          Number of times each of the thread should execute action
   * @param timeoutInExecute
   *          How much to wait in action execution
   */
  ...
  class StressTestThread extends Thread {
     ...
     * 
     * @param pool
     *          Pool to get clients from
     * @param id
     *          Id of this thread (for debug purposes only)
     * @param nLoops
     *          Number of iteration loops
     * @param startGate
     *          Latch gate that tells this thread it can start iteration
     * @param endGate
     *          Latch gate that callback this thread ended execution
     */
