...
public class TestServiceCalls extends TestBase {
  ...
  /**
   * This is utility method that checks if the {@link TransactionManager#begin(boolean)},
   * {@link TransactionManager#commit()}, {@link TransactionManager#rollback()} are called properly. If the transaction
   * is finished normally begin() and commit() are called and rollback() is not called
   */
   ...
   * This is utility method that checks if the {@link TransactionManager#begin(boolean)},
   * {@link TransactionManager#commit()}, {@link TransactionManager#rollback()} are called properly. If the transaction
   * not finished normally begin() and rollback() are called and commit() is not called
   */
