...
public class MozzesServerConfiguration {
	 ...
	 * 
	 * @param <I> serviceInterface that specifies service behavior.
	 * 
	 * @param serviceInterface Class object of serviceInterface.
	 * @param serviceImplementation Class object of the class that implements serviceInterface
	 */
	...
	private class MozzesServerModule extends AbstractModule {
		...
		private MozzesServerModule(InternalInjectorProvider iip) {
			...
			TRANSACTION = new AbstractContextScope("MozzesScopes.TRANSACTION", new TransactionContextProvider(
					requestManager));
		}
		...
		protected void configure() {
			...
			bind(SessionManager.class).annotatedWith(MozzesInternal.class).to(SessionManagerImpl.class).in(
					Scopes.SINGLETON);
		...
		private void bindModule(Binder binder, ApplicationModule module, LookupServiceImpl lookupService,
			...
			if (services != null) {
				final ServiceChainConfiguration scc = new ServiceChainConfiguration(iip).addInterceptors(
						globalInterceptors).addInterceptors(module.getServiceInterceptors());
		...
		private <I> void bindService(Binder binder, ServiceConfiguration<I> serviceConfig, ServiceChainConfiguration scc) {
			...
			binder.bind(serviceInterface).toProvider(
					new ServiceProvider<I>(serviceInterface, serviceImplementationKey, scc));
		}
	}
	...
	private static class ServiceProvider<I> implements Provider<I> {
		...
		ServiceProvider(Class<I> serviceInterface, Key<? extends I> serviceImplementationKey,
				ServiceChainConfiguration scc) {
