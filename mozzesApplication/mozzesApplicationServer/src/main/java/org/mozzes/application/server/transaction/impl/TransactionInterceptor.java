...
public class TransactionInterceptor implements InvocationInterceptor {
	...
	/**
	 * @param transactionStackProvider Stack of transactions that are associated with the request.<br>
	 *            {@link TransactionStack} is bound to the MozzesScopes RequestScope so it's always the same in
	 *            the same request
	 */
	@Inject
	...
	public <I> Object invoke(Invocation<? super I> invocation, InvocationHandler<I> target) throws Throwable {
		...
		try {
			...
			try {
				...
				/*
				 * after invocation if we're here that means the transaction didn't throw any exception and therefore
				 * it's successful
				 */
				trasactionSuccessful = true;
			} catch (Throwable thr) {
				/*
				 * if transaction threw exception we will propagate the exception but only if that's not the "normal"
