...
public class ServiceWithRequestDataImpl implements ServiceWithRequestData {
   ...
   * the some other service that also works with that value would preserve value because both service invocations are in
   * the same request.(there's only one "root" service method invocation in the request but that method can call some
   * other services. (in this example actionWithRequestContext() is the root service invocation and
